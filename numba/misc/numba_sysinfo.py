...
def get_sysinfo():
    ...
    sys_info = {
        ...
        _numba_env_vars: {
            k: v for (k, v) in os.environ.items() if k.startswith("NUMBA_")
        },
...
def display_sysinfo(info=None, sep_pos=45):
    ...
    template = (
        ...
        (
            ...
            "\n".join(
                ...
                for i, l in enumerate(
                    textwrap.wrap(info.get(_cpu_features, "?"), width=79 - sep_pos)
                )
            ),
        ),
        ("",),
        ...
        (
            ...
            DisplaySeq(
                info.get(_numpy_supported_simd_features, []) or ("None found.",)
            ),
        ),
        (
            "NumPy Supported SIMD dispatch",
            DisplaySeq(
                info.get(_numpy_supported_simd_dispatch, []) or ("None found.",)
            ),
        ),
        (
            "NumPy Supported SIMD baseline",
            DisplaySeq(
                info.get(_numpy_supported_simd_baseline, []) or ("None found.",)
            ),
        ),
        (
        ...
        (
            DisplayMap({k: v for k, v in info.items() if k.startswith("Conda")})
            or ("Conda not available.",)
        ),
        ("",),
    ...
    for t in template:
        if hasattr(t, "display_seq_flag"):
            ...
            for d in t:
                print(
                    *tuple(fmt % ("\t" + k, v) for (k, v) in d.items()),
                    sep="\n",
                    end="\n",
                )
        elif len(t) == 2:
