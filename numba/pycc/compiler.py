...
class _ModuleCompiler(object):
    ...
    #: };
    method_def_ty = ir.LiteralStructType(
    ...
    env_def_ty = ir.LiteralStructType(
        (lt._void_star, lt._int32, lt._void_star, lt._void_star, lt._int32)
    )
    ...
    def _emit_method_array(self, llvm_module):
        ...
        method_array = cgutils.add_global_variable(
            llvm_module, method_array_init.type, ".module_methods"
        )
    ...
    def _emit_envgvs_array(self, llvm_module, builder, pyapi):
        ...
        env_setters.extend(
            [
                self.context.declare_env_global(llvm_module, envgv_name).bitcast(
                    lt._void_star
                )
...
class ModuleCompiler(_ModuleCompiler):
    ...
    def module_create_definition(self):
        ...
        signature = ir.FunctionType(
            lt._pyobject_head_p, (ir.PointerType(self.module_def_ty), lt._int32)
        )
    ...
    def _emit_python_wrapper(self, llvm_module):
        ...
        mod_def_base = cgutils.add_global_variable(
            llvm_module, mod_def_base_init.type, ".module_def_base"
        )
        ...
        # Define a constant string for the module name.
        mod_def = cgutils.add_global_variable(
            llvm_module, mod_def_init.type, ".module_def"
        )
        ...
        mod = builder.call(
            create_module_fn, (mod_def, ir.Constant(lt._int32, sys.api_version))
        )
