...
class Test(TestCase):
    def test(self):
        ...
        expect = (
            '[frame={level="0",args=[{name="x",type="Literal[int](10)",value="10"}]}]'
        )
        ...
        driver.stack_list_arguments(2)
        expect = (
            '[frame={level="0",args=[{name="x",type="Literal[int](20)",value="20"}]}]'
        )
        driver.assert_output(expect)
        ...
        driver.stack_list_arguments(2)
        expect = (
            '[frame={level="0",args=[{name="x",type="Literal[int](30)",value="30"}]}]'
        )
        driver.assert_output(expect)
        driver.quit()
