...
class TestUpdateInplace(TestCase):
    ...
    def test_update_inplace(self):
        ...
        gufunc = guvectorize(
            ["void(f8[:], f8[:])"], "(t),()", nopython=True, writable_args=(0,)
        )(py_replace_2nd)
        self._run_test_for_gufunc(gufunc, py_replace_2nd)

        # test with writable_args as strings
        gufunc = guvectorize(
            ["void(f8[:], f8[:])"], "(t),()", nopython=True, writable_args=("x_t",)
        )(py_replace_2nd)
        self._run_test_for_gufunc(gufunc, py_replace_2nd)
    ...
    def test_update_inplace_3(self):
        # test without writable_args
        gufunc = guvectorize(
            ["void(f8[:], f8[:], f8[:], f8[:])"], "(t),(t),(t),()", nopython=True
        )(py_update_3)
    ...
    def test_exceptions(self):
        ...
        with self.assertRaises(ValueError):
            guvectorize(
                ...
                "(t),()",
                nopython=True,
                writable_args=(0, 1, 2, 5),
            )(py_replace_2nd)

        # check that all values in writable_args are between 0 and nin
        with self.assertRaises(ValueError):
            guvectorize(
                ["void(f8[:], f8[:])"], "(t),()", nopython=True, writable_args=(5,)
            )(py_replace_2nd)

        with self.assertRaises(ValueError):
            guvectorize(
                ["void(f8[:], f8[:])"], "(t),()", nopython=True, writable_args=(-1,)
            )(py_replace_2nd)

        # check that exception is raised when passing non-existing argument name
        with self.assertRaises(RuntimeError):
            guvectorize(
                ["void(f8[:], f8[:])"], "(t),()", nopython=True, writable_args=("z_t",)
            )(py_replace_2nd)
