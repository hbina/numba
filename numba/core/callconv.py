...
class CPUCallConv(BaseCallConv):
    ...
    def emit_wrap_args_insts(self, builder, pyapi, struct_type, exc_args):
        ...
        st_ptr = builder.bitcast(
            self.context.nrt.allocate(builder, st_size), struct_type.as_pointer()
        )
    ...
    def set_dynamic_user_exc(
        self, builder, exc, exc_args, nb_types, loc=None, func_name=None
    ):
        """
        ...
        struct_type = ir.LiteralStructType(
            [arg.type for arg in exc_args if isinstance(arg, ir.Value)]
        )
        ...
        excinfo_p = builder.bitcast(
            self.context.nrt.allocate(builder, exc_size), excinfo_ptr_t
        )
    ...
    def return_dynamic_user_exc(
        self, builder, exc, exc_args, nb_types, loc=None, func_name=None
    ):
        """
        Same as ::return_user_exc but for dynamic exceptions
        """
        self.set_dynamic_user_exc(
            builder, exc, exc_args, nb_types, loc=loc, func_name=func_name
        )
        self._return_errcode_raw(builder, RETCODE_USEREXC)
