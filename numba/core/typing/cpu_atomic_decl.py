...
@register
class CpuAtomicFetchAdd(AbstractTemplate):
...
def _gen_cpu_atomic_cas(l_key, supported_types):
    @register
    class CpuAtomicArray(AbstractTemplate):
        key = l_key
...
CpuAtomicFetchAddArray = _gen_cpu_atomic_rmw(atomic.fetch_add, atomic_integer_types)
CpuAtomicCompareAndSwapArray = _gen_cpu_atomic_cas(
...
# Register atomic directly as a module so "from numba.cpu import atomic" works
...
class CPUAtomicTemplate(AttributeTemplate):
    ...
    def resolve_compare_and_swap(self, mod):
        return types.Function(CpuAtomicCompareAndSwapArray)

    def resolve_fetch_add(self, mod):
        return types.Function(CpuAtomicFetchAddArray)
