...
def atomic_binary_1dim_shared(
    ary, idx, op2, ary_dtype, ary_nelements, binop_func, cast_func, initializer, neg_idx
):
...
def atomic_add_float(ary):
    atomic_binary_1dim_shared(
        ary, ary, 1.0, float32, 32, cuda.atomic.add, atomic_cast_to_int, 0.0, False
    )


def atomic_add_float_wrap(ary):
    atomic_binary_1dim_shared(
        ary, ary, 1.0, float32, 32, cuda.atomic.add, atomic_cast_to_int, 0.0, True
    )
...
def atomic_add_double(idx, ary):
    atomic_binary_1dim_shared(
        ary, idx, 1.0, float64, 32, cuda.atomic.add, atomic_cast_none, 0.0, False
    )
...
def atomic_sub_float(ary):
    atomic_binary_1dim_shared(
        ary, ary, 1.0, float32, 32, cuda.atomic.sub, atomic_cast_to_int, 0.0, False
    )
...
def atomic_sub_double(idx, ary):
    atomic_binary_1dim_shared(
        ary, idx, 1.0, float64, 32, cuda.atomic.sub, atomic_cast_none, 0.0, False
    )
...
class TestCudaAtomics(CUDATestCase):
    ...
    def check_dec_index(self, ary, idx, rconst, sig, nblocks, blksize, func):
        ...
        np.testing.assert_equal(
            ary, np.where(orig == 0, rconst, np.where(orig > rconst, rconst, orig - 1))
        )
    ...
    def test_atomic_compare_and_swap(self):
        self.check_cas(
            n=100, fill=-99, unfill=-1, dtype=np.int32, cas_func=atomic_compare_and_swap
        )

    def test_atomic_compare_and_swap2(self):
        self.check_cas(
            n=100, fill=-45, unfill=-1, dtype=np.int64, cas_func=atomic_compare_and_swap
        )
    ...
    def test_atomic_cas_2dim(self):
        self.check_cas(
            n=100, fill=-99, unfill=-1, dtype=np.int32, cas_func=atomic_cas_2dim, ndim=2
        )
    ...
    def test_atomic_cas2_2dim(self):
        self.check_cas(
            n=100, fill=-45, unfill=-1, dtype=np.int64, cas_func=atomic_cas_2dim, ndim=2
        )
    ...
    def test_atomic_cas3_1dim(self):
        ...
        runfill = np.random.randint(1, 25, dtype=np.uint32)
        self.check_cas(
            n=100, fill=rfill, unfill=runfill, dtype=np.uint32, cas_func=atomic_cas_1dim
        )
    ...
    def test_atomic_cas4_1dim(self):
        ...
        runfill = np.random.randint(1, 25, dtype=np.uint64)
        self.check_cas(
            n=100, fill=rfill, unfill=runfill, dtype=np.uint64, cas_func=atomic_cas_1dim
        )
