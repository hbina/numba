...
def make_gufunc_kernel(_dufunc):
    ...
    class GUFuncKernel(npyimpl._Kernel):
        ...
        def __init__(self, context, builder, outer_sig):
            ...
            self.inner_sig, self.cres = self.dufunc.find_ewise_function(ewise_types)

        def cast(self, val, fromty, toty):
            # Handle the case where "fromty" is an array and "toty" a scalar
            if isinstance(fromty, types.Array) and not isinstance(toty, types.Array):
        ...
        def generate(self, *args):
            if self.cres.objectmode:
                msg = (
                    "Calling a guvectorize function in object mode is not "
                    "supported yet."
                )
...
class GUFunc(serialize.ReduceMixin, UfuncBase):
    ...
    def _get_ewise_dtypes(self, args):
        argtys = map(
            lambda arg: arg if isinstance(arg, types.Type) else typeof(arg), args
        )
        tys = []
        for argty in argtys:
    ...
    def __call__(self, *args, **kwargs):
        ...
        if self._frozen or not self.is_dynamic:
            ...
            if args and _is_array_wrapper(args[0]):
                return args[0].__array_ufunc__(self, "__call__", *args, **kwargs)
            else:
