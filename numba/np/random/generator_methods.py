...
def _get_proper_func(func_32, func_64, dtype, dist_name="the given"):
    ...
    if np_dt not in [np.float32, np.float64]:
        raise TypingError(
            "Argument dtype is not one of the"
            + " expected type(s): "
...
def check_types(obj, type_list, arg_name):
    ...
    if not any([isinstance(obj, _type) for _type in type_list]):
        raise TypingError(
            f"Argument {arg_name} is not one of the" + f" expected type(s): {type_list}"
        )


# Overload the Generator().integers()
@overload_method(types.NumPyRandomGeneratorType, "integers")
def NumPyRandomGeneratorType_integers(
    inst, low, high, size=None, dtype=np.int64, endpoint=False
):
    ...
    if is_nonelike(size):
        ...
        def impl(inst, low, high, size=None, dtype=np.int64, endpoint=False):
            random_methods._randint_arg_check(
                low, high, endpoint, lower_bound, upper_bound
            )
            if not endpoint:
                high -= dtype(1)
                low = dtype(low)
...
def NumPyRandomGeneratorType_standard_exponential(
    inst, size=None, dtype=np.float64, method="zig"
):
    check_types(method, [types.UnicodeType, str], "method")
    dist_func_inv, nb_dt = _get_proper_func(
        random_standard_exponential_inv_f, random_standard_exponential_inv, dtype
    )

    dist_func, nb_dt = _get_proper_func(
        random_standard_exponential_f, random_standard_exponential, dtype
    )
...
def NumPyRandomGeneratorType_standard_normal(inst, size=None, dtype=np.float64):
    dist_func, nb_dt = _get_proper_func(
        random_standard_normal_f, random_standard_normal, dtype
    )
    if isinstance(size, types.Omitted):
        size = size.value
...
def NumPyRandomGeneratorType_standard_gamma(inst, shape, size=None, dtype=np.float64):
    ...
    dist_func, nb_dt = _get_proper_func(
        random_standard_gamma_f, random_standard_gamma, dtype
    )
    if isinstance(size, types.Omitted):
        size = size.value
...
def NumPyRandomGeneratorType_noncentral_f(inst, dfnum, dfden, nonc, size=None):
    ...
    if is_nonelike(size):
        ...
        def impl(inst, dfnum, dfden, nonc, size=None):
            ...
            return np.float64(
                random_noncentral_f(inst.bit_generator, dfnum, dfden, nonc)
            )
