...
def get_ext_modules():
    ...
    ext_devicearray = Extension(
        name="numba._devicearray",
        sources=["numba/_devicearray.cpp"],
        depends=["numba/_pymodule.h", "numba/_devicearray.h"],
        include_dirs=["numba"],
    ...
    ext_dispatcher = Extension(
        ...
        depends=["numba/_pymodule.h", "numba/_typeof.h", "numba/_hashtable.h"],
        extra_compile_args=["-std=c++11"],
        **np_compile_args,
    )
    ...
    ext_helperlib = Extension(
        ...
        extra_link_args=install_name_tool_fixer + extra_link_args,
        depends=[
            "numba/_pymodule.h",
            "numba/_helperlib.c",
            "numba/_lapack.c",
    ...
    ext_typeconv = Extension(
        name="numba.core.typeconv._typeconv",
        sources=[
            "numba/core/typeconv/typeconv.cpp",
    ...
    ext_np_ufunc = Extension(
        name="numba.np.ufunc._internal",
    ...
    ext_npyufunc_num_threads = Extension(
        name="numba.np.ufunc._num_threads",
        sources=["numba/np/ufunc/_num_threads.c"],
        depends=["numba/_pymodule.h"],
    )
    ...
    ext_np_ufunc_workqueue_backend = Extension(
        ...
        sources=["numba/np/ufunc/workqueue.c", "numba/np/ufunc/gufunc_scheduler.cpp"],
        depends=["numba/np/ufunc/workqueue.h"],
        extra_link_args=extra_link_args,
    )
    ...
    ext_nrt_python = Extension(
        name="numba.core.runtime._nrt_python",
        sources=["numba/core/runtime/_nrt_pythonmod.c", "numba/core/runtime/nrt.cpp"],
        depends=[
    ...
    ext_jitclass_box = Extension(
        name="numba.experimental.jitclass._box",
        sources=["numba/experimental/jitclass/_box.c"],
        depends=["numba/experimental/_pymodule.h"],
    )

    ext_cuda_extras = Extension(
        name="numba.cuda.cudadrv._extras",
...
metadata = dict(
    ...
    package_data={
        ...
        "numba.cuda": ["cpp_function_wrappers.cu", "cuda_fp16.h", "cuda_fp16.hpp"],
    },
    scripts=["bin/numba"],
